	...
	fn new(input: &str) -> Self {
		...
		for (y, line) in input.lines().enumerate() {
			for (x, tile) in line.bytes().enumerate() {
				match tile {
					b'.' => {map.insert((x as i32, y as i32),  Tile::Space);},
					b'#' => {map.insert((x as i32, y as i32),  Tile::Wall);},
					_ => {},
				}
			}
		}
...
impl Path {
	fn new(input: &str) -> Self {
		...
		let mut parser = many0(alt((map(parse_i32, Inst::Forward), value(Inst::Left, tag("L")), value(Inst::Right, tag("R")))));
...
struct Cursor {
	loc: (i32, i32),
	dir: u8,
	min_x: HashMap<i32, (i32, i32)>,
	max_x: HashMap<i32, (i32, i32)>,
	min_y: HashMap<i32, (i32, i32)>,
	max_y: HashMap<i32, (i32, i32)>,
}

impl Cursor {
	fn new(map: &Map) -> Self {
		let (&loc, _) = map.0.iter().filter(|((_, y), t)| *y == 0 && **t == Tile::Space).min_by_key(|((x, _), _)| x).unwrap();
		Self {
			loc,
			dir: 0,
			..Self::default()
				...
				for _ in 0..num {
					...
					let new_tile = match self.dir {
						0 => {
							...
							map.0.get(&new_loc).or_else(|| {
								new_loc = *self.min_x.entry(self.loc.1).or_insert_with(|| {
									*map.0.keys().filter(|(_, y)| *y == self.loc.1).min_by_key(|(x, _)| x).unwrap()
								});
								map.0.get(&new_loc)
							}).unwrap()
						}
						1 => {
							...
							map.0.get(&new_loc).or_else(|| {
								new_loc = *self.min_y.entry(self.loc.0).or_insert_with(|| {
									*map.0.keys().filter(|(x, _)| *x == self.loc.0).min_by_key(|(_, y)| y).unwrap()
								});
								map.0.get(&new_loc)
							}).unwrap()
						}
						2 => {
							new_loc = (self.loc.0 - 1, self.loc.1);
							map.0.get(&new_loc).or_else(|| {
								new_loc = *self.max_x.entry(self.loc.1).or_insert_with(|| {
									*map.0.keys().filter(|(_, y)| *y == self.loc.1).max_by_key(|(x, _)| x).unwrap()
								});
								map.0.get(&new_loc)
							}).unwrap()
						}
						3 => {
							new_loc = (self.loc.0, self.loc.1 - 1);
							map.0.get(&new_loc).or_else(|| {
								new_loc = *self.max_y.entry(self.loc.0).or_insert_with(|| {
									*map.0.keys().filter(|(x, _)| *x == self.loc.0).max_by_key(|(_, y)| y).unwrap()
								});
								map.0.get(&new_loc)
							}).unwrap()
						}
	...
	fn walk2(&mut self, inst: Inst, map: &Map) {
		match inst {
			Inst::Left => self.dir = (self.dir + 3) % 4,
			Inst::Right => self.dir = (self.dir + 1) % 4,
			Inst::Forward(num) => {
	...
	fn walk_all2(&mut self, insts: impl IntoIterator<Item = Inst>, map: &Map) {
		for inst in insts {
			self.walk(inst, map);
		}
	}
